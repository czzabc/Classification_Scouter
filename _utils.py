...
def weighted_cross_entropy_loss(pred, true, weights=None):
    # Reshape the prediction to (batch_size * n_genes, 3) and true labels to (batch_size * n_genes)
    pred_reshaped = pred.view(-1, 3)
    true_reshaped = true.long().view(-1)

    # The cross_entropy loss function in PyTorch expects class indices, so we map our labels (-1, 0, 1) to (0, 1, 2)
    # Down-regulated (-1) -> 0
    # Unchanged (0) -> 1
    # Up-regulated (1) -> 2
    true_mapped = true_reshaped + 1

    loss_fn = nn.CrossEntropyLoss(weight=weights)
    loss = loss_fn(pred_reshaped, true_mapped)
    return loss


def focal_loss(pred, true, alpha=None, gamma=2.0):
    '''
    Parameters:
    - pred (torch.Tensor):
        The model's predictions (logits). Shape: (batch_size, n_genes * 3).
    - true (torch.Tensor):
        The ground truth labels. Shape: (batch_size, n_genes).
    - alpha (torch.Tensor, optional):
        A tensor of weights for each class. Shape: (3,). If None, no weighting is applied.
        Defaults to None.
    - gamma (float, optional):
        The focusing parameter. Defaults to 2.0.
    '''
    pred_reshaped = pred.view(-1, 3)
    true_reshaped = true.long().view(-1)
    true_mapped = true_reshaped + 1
    ...
    ce_loss = F.cross_entropy(pred_reshaped, true_mapped, reduction='none')
    p = torch.exp(-ce_loss)
